...
import os
import torch
import nrrd  # Import the nrrd library

def construct_user(model, loss_fn, cfg_case, setup):
    """Interface function."""
    
    print('loss_fn',loss_fn)
    print('cfg_case',cfg_case)
    print('setup',setup)
...
class UserSingleStep(torch.nn.Module):
    ...
    def plot(self, user_data, scale=False, print_labels=False):
        ...
        if data.shape[0] == 1:
            ...
            if print_labels:
                print(label_classes)
        #self.save_as_nifti(user_data, scale)
        self.save_as_nrrd(user_data, scale)

    # def save_as_nifti(self, user_data, scale=False):
    #     import os
    #     #import torch
    ...
    #     data_array = data.permute(2, 3, 0, 1).cpu().numpy()

    #     # Create a NIfTI image
    #     img = nib.Nifti1Image(data_array, affine=None)

    #     # Determine the filename based on the original data (assuming user_data contains a key "filename")
    #     original_filename = user_data.get("filename", "reconstructed")
    #     output_filename = f"{os.path.splitext(original_filename)[0]}_processed.nii.gz"

    #     # Save the NIfTI image to the local directory
    #     nib.save(img, output_filename)

    #     print(f"NIfTI file saved at: {output_filename}")
        
    def save_as_nrrd(self, user_data, scale=False):
        ...
        # Determine the filename based on the original data (assuming user_data contains a key "filename")
        original_filename = user_data.get("filename", "reconstructed")
        output_filename = f"{os.path.splitext(original_filename)[0]}_processed.nrrd"

        # Save the data array as an NRRD file
        nrrd.write(output_filename, data_array)

        print(f"NRRD file saved at: {output_filename}")
