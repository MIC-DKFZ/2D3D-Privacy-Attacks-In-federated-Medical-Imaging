...
def _construct_vision_model(cfg_model, cfg_data, pretrained=True, **kwargs):
    ...
    if "ImageNet" in cfg_data.name:
        ...
        elif "brain_tumor_model" == cfg_model.lower():
          
            model = Brain_tumor_model( num_channels=channels, num_classes=classes,width=64)
        elif "my3dnet" == cfg_model.lower():
          
            model = MY3DNet( num_channels=channels, num_classes=classes)
        else:
...
class Brain_tumor_model(torch.nn.Module):
    """Modified ConvNet for 4-class classification with input dimension 512x512."""


    def __init__(self, num_classes=4, num_channels=3, width=64):
        """Init with width and num classes."""
        super().__init__()
        self.model = torch.nn.Sequential(
            OrderedDict(
...
class MY3DNet(nn.Module):
    def __init__(self, num_channels, num_classes):
        ...
        self.conv1 = nn.Conv3d(num_channels, 64, kernel_size=3, padding=1)
